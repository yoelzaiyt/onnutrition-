...
const QUESTIONNAIRES: Questionnaire[] = [
  {
    ...
    description:
      "Avalia obesidade, resistência à insulina e síndrome metabólica.",
    icon: Stethoscope,
    color: "text-blue-500",
    questions: [
  ...
  },
  {
    ...
    description:
      "Detecta compulsão alimentar, comer emocional e falta de controle.",
    icon: Brain,
...
export default function HealthQuestionnaires({
  patientId,
}: {
  patientId: string;
}) {
  const [view, setView] = useState<"dashboard" | "quiz" | "result">(
    "dashboard",
  );
    ...
    const mockResults: QuestionnaireResult[] = [
      {
        ...
        aiAnalysis:
          "O paciente apresenta um risco metabólico moderado a alto, com destaque para fadiga e sono de má qualidade. A circunferência abdominal elevada sugere resistência à insulina incipiente. Recomenda-se foco em higiene do sono e controle glicêmico.",
      ...
      },
      {
        ...
        aiAnalysis:
          "Comportamento alimentar com traços de comer emocional e culpa pós-refeição. É necessário trabalhar a relação com a comida e técnicas de mindful eating.",
        lastUpdated: new Date().toISOString(),
      },
      {
        ...
        aiAnalysis:
          "Sintomas digestivos significativos, principalmente constipação e gases. Sugere-se aumento na ingestão de fibras solúveis e hidratação, além de avaliação de intolerâncias alimentares.",
        lastUpdated: new Date().toISOString(),
      },
    ];
  ...
  useEffect(() => {
    ...
    const q = query(
      collection(db, "patients", patientId, "questionnaireResults"),
    );
  ...
  const saveResult = async (result: QuestionnaireResult) => {
    ...
    try {
      ...
      handleFirestoreError(
        error,
        OperationType.WRITE,
        `patients/${patientId}/questionnaireResults`,
      );
    }
  };
  ...
  const generateAIAnalysis = async (
    ...
    try {
      ...
      const analysis =
        response.text || "Não foi possível gerar a análise no momento.";
    ...
    return (
      <div className="p-8 space-y-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            <h2 className="text-3xl font-black text-[#0B2B24]">
              Questionários de Saúde
            </h2>
            <div className="flex items-center gap-4">
              <p className="text-slate-400 font-bold text-sm">
                Triagem clínica avançada e monitoramento de riscos.
              </p>
              <button
          ...
          <div className="flex gap-4">
            <div className="bg-white p-4 rounded-3xl border border-slate-100 shadow-sm flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xl font-black text-[#0B2B24]">
                  {generalHealthScore}
                </p>
              </div>
            </div>
          ...
          <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6">
            {QUESTIONNAIRES.map((quiz) => {
              ...
              const classification = result
                ? getClassification(result.score)
                : null;
                ...
                >
                  <div>
                    <div className="flex justify-between items-start mb-4">
                      <div
                        className={`w-12 h-12 rounded-2xl bg-slate-50 flex items-center justify-center ${quiz.color}`}
                      >
                        <quiz.icon className="w-6 h-6" />
                      </div>
                      {result ? (
                        <div
                          className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${classification?.bg} ${classification?.color}`}
                        >
                    ...
                    <h3 className="text-lg font-black text-[#0B2B24] mb-2">
                      {quiz.title}
                    </h3>
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-8 max-w-2xl mx-auto w-full">
          <div className="w-full mb-12">
            <div className="flex justify-between items-end mb-2">
              <span className="text-[10px] font-black text-[#22B391] uppercase tracking-widest">
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <h2 className="text-3xl font-black text-[#0B2B24] leading-tight">
      ...
      <div className="p-8 max-w-6xl mx-auto space-y-8">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-400 font-bold text-sm">
              Análise concluída para {selectedQuiz.title}.
            </p>
          </div>
          <button
            onClick={() => setView("dashboard")}
            ...
            <div className="bg-white p-8 rounded-[40px] shadow-xl shadow-slate-200/50 flex flex-col items-center justify-center text-center border border-slate-50">
              <div className="relative w-48 h-48 mb-6">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  <span className="text-5xl font-black text-[#0B2B24]">
                    {result.score}
                  </span>
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                    Pontos
              ...
              <div
                className={`px-6 py-2 rounded-full text-xs font-black uppercase tracking-widest mb-2 ${classification.bg} ${classification.color}`}
              >
                {classification.label}
              </div>
              <p className="text-xs text-slate-400 font-bold max-w-[200px]">
                O score indica o nível de atenção necessário para esta
                categoria.
              </p>
            </div>
            ...
            <div className="bg-[#0B2B24] p-10 rounded-[48px] text-white relative overflow-hidden">
              ...
              <div className="relative z-10">
                <div className="flex items-center gap-4 mb-8">
                  ...
                  <div>
                    <h3 className="text-2xl font-black">Análise Nutricional</h3>
                ...
                <div className="p-8 bg-white/5 rounded-[32px] border border-white/10 backdrop-blur-sm min-h-[120px] flex items-center justify-center">
                  {isAnalyzing ? (
                    <div className="flex flex-col items-center gap-4">
                      <Loader2 className="w-8 h-8 text-[#22B391] animate-spin" />
                      <p className="text-sm font-bold text-emerald-400/60 uppercase tracking-widest animate-pulse">
                        Analisando com IA...
                      </p>
                    </div>
                  ) : (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-white p-8 rounded-[40px] border border-slate-100 shadow-sm">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <h4 className="text-sm font-black text-[#0B2B24] uppercase tracking-widest">
                    Recomendações
                  </h4>
                </div>
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-emerald-600/70 font-bold">
                    Enviar relatório completo para o nutricionista.
                  </p>
                </div>
              </div>
              <button className="px-8 py-4 bg-[#22B391] text-white rounded-2xl font-black uppercase tracking-widest text-[10px] shadow-lg shadow-[#22B391]/20 hover:scale-105 transition-all">
                Enviar Agora
              </button>
            </div>
          </div>
