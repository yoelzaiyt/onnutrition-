...
export const FirebaseProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const [mockUser, setMockUserInternal] = useState<any | null>({
    ...
    try {
      ...
      unsubscribe = onAuthStateChanged(auth, async (user) => {
        try {
          ...
          if (user && user.email !== 'word.intelligence@gmail.com') {
            console.warn('FirebaseProvider: Non-admin user detected:', user.email);
            setAuthError('Acesso restrito apenas para administradores.');
            updateDiag('auth-denied-non-admin');
            await signOut(auth);
            setUser(null);
          } else {
            if (user) console.log('FirebaseProvider: Admin user confirmed:', user.email);
            setUser(user);
            updateDiag(user ? 'auth-user-found' : 'auth-user-null');
          }
