...
import { Leaf, Mail, Lock, Eye, EyeOff, ArrowRight, User, Users } from 'lucide-react';
...
export const LoginPage: React.FC<AuthProps> = ({ onLogin, onGoToRegister, onBackToLanding }) => {
  ...
  const isDemoEmail = email.toLowerCase() === 'demo@at.com' || email.toLowerCase() === 'demo';
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex flex-col items-center justify-center p-6 font-sans relative overflow-hidden">
      {/* Background Pattern */}
      <div className="absolute inset-0 opacity-[0.03] pointer-events-none" style={{ backgroundImage: 'radial-gradient(#22B391 1px, transparent 1px)', backgroundSize: '32px 32px' }}></div>
      ...
      >
        <div className="flex justify-center mb-10 cursor-pointer" onClick={onBackToLanding}>
        ...
        <div className="bg-white rounded-[2.5rem] shadow-2xl shadow-slate-200/60 p-10 md:p-12 border border-slate-100">
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            <div className="space-y-2">
              ...
              <div className="relative group">
                ...
                <input 
                  ...
                  className="w-full pl-12 pr-4 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-[#22B391]/20 focus:border-[#22B391] transition-all"
                  value={email}
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center ml-1">
                ...
                <button type="button" className="text-xs font-bold text-[#22B391] hover:underline">Esqueceu a senha?</button>
              </div>
              <div className="relative group">
                ...
                <input 
                  ...
                  placeholder="••••••••"
                  className="w-full pl-12 pr-12 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-[#22B391]/20 focus:border-[#22B391] transition-all"
                  value={password}
            ...
            >
              {isLoading ? (
                ...
                <>{!hasConfig ? 'Entrar (Modo Demo)' : 'Entrar'} <ArrowRight className="w-5 h-5" /></>
              )}
            </button>
            {(!hasConfig || isDemoEmail) && (
              <p className="text-xs text-amber-600 text-center font-bold mt-2 bg-amber-50 p-2 rounded-lg border border-amber-100">
                ✨ Você está no Modo Demo. Use qualquer email e senha para entrar.
              </p>
            )}
          </form>
          ...
          <div className="mt-10 text-center">
            <p className="text-slate-500 font-medium">
              Não tem uma conta? <button onClick={onGoToRegister} className="text-[#22B391] font-bold hover:underline">Criar conta grátis</button>
            </p>
          </div>
        </div>

        <p className="mt-10 text-center text-xs text-slate-400 leading-relaxed max-w-[320px] mx-auto">
          Ao entrar, você concorda com nossos <a href="#" className="text-[#22B391] hover:underline">Termos de Uso</a> e <a href="#" className="text-[#22B391] hover:underline">Política de Privacidade</a>
        </p>
      </motion.div>
    </div>
  );
...
export const RegisterPage: React.FC<AuthProps> = ({ onLogin, onGoToLogin, onBackToLanding }) => {
  const [selectedType, setSelectedType] = useState<'nutri' | 'patient' | null>(null);
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (authData.user) {
        const finalRole = email === 'word.imtelligence@gmail.com' ? 'nutri' : selectedType;
      ...
      if (isNetworkError) {
        setError('Erro de conexão: Verifique se as chaves do Supabase estão preenchidas no arquivo .env ou no painel de Segredos (⚙️).');
        ...
        <div className="bg-white rounded-[2rem] shadow-2xl shadow-slate-200/60 p-10 border border-slate-100 text-center">
          ...
          {step === 1 ? (
            <div className="flex gap-4 mb-8">
              ...
              >
                <Users className={`w-8 h-8 ${selectedType === 'nutri' ? 'text-[#22B391]' : 'text-slate-400'}`} />
              ...
              >
                <User className={`w-8 h-8 ${selectedType === 'patient' ? 'text-[#22B391]' : 'text-slate-400'}`} />
            ...
            <form className="space-y-6 text-left" onSubmit={handleRegister}>
              <div className="space-y-2">
                ...
                <input 
                  ...
                  className="w-full px-4 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-[#22B391]/20 focus:border-[#22B391] transition-all"
                  value={name}
          ...
          <div className="flex gap-4">
            ...
            >
              {isLoading ? (
                ...
                <>
                  {step === 1 ? 'Continuar' : (!hasConfig ? 'Criar Conta (Modo Demo)' : 'Criar Conta')}
          ...
          <div className="mt-8 text-center">
            <p className="text-slate-500 font-medium">
              Já tem uma conta? <button onClick={onGoToLogin} className="text-[#22B391] font-bold hover:underline">Entrar</button>
            </p>
          </div>
        </div>

        <p className="mt-10 text-center text-xs text-slate-400 leading-relaxed max-w-[420px] mx-auto">
          Ao criar conta, você concorda com nossos <a href="#" className="text-[#22B391] hover:underline">Termos de Uso</a> e <a href="#" className="text-[#22B391] hover:underline">Política de Privacidade</a>
        </p>
      </motion.div>
    </div>
  );
