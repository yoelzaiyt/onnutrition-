...
import { medicalRecordService } from '@/app/lib/medicalRecordService';
  ...
  useEffect(() => {
    const fetchPatient = async () => {
      ...
      try {
        ...
        if (error) {
          console.warn('Patient not found in Supabase, using demo data:', error.message);
          setPatient({
            id: patientId,
            name: 'Maria Oliveira',
            email: 'maria@email.com',
                ...
                onSave={async (data) => {
                  try {
                    ...
                    const summary = `
                      ...
                      Estresse: ${data.nivel_estresse}/5 | Sono: ${data.qualidade_sono}/5 (${data.horas_sono}h)
                      Peso Atual: ${data.peso_atual}kg | Habitual: ${data.peso_habitual}kg | Ideal: ${data.peso_ideal}kg
                      Doenças: ${data.doencas.join(', ')}
                      Problemas Gastro: ${data.problemas_gastro.join(', ')}
                      Medicamentos: ${data.medicamentos}
                      Suplementos: ${data.suplementos.join(', ')}
                      Hábitos: ${data.refeicoes_dia} refeições/dia, Açúcar: ${data.consumo_acucar}, Refri: ${data.consumo_refri}
                      Exercício: ${data.pratica_exercicio ? `${data.tipo_exercicio.join(', ')} (${data.frequencia_semanal}x/semana)` : 'Não pratica'}
                      Água: ${data.agua_dia}L/dia
                    `.trim();

                    await medicalRecordService.upsertMedicalRecord({
                      patient_id: patientId,
                      nutri_id: user?.id || 'demo-nutri',
                      clinical_history: summary,
                      nutritional_diagnosis: data.diagnostico_inicial || 'Aguardando avaliação detalhada.'
                    });

                    toast.success('Anamnese salva com sucesso no prontuário!');
                    setActiveTab('perfil');
                  } catch (error) {
