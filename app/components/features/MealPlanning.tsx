...
import { DietPlan, Meal, MealFood, dietPlanService } from '@/app/lib/dietPlanService';
...
const MealPlanning: React.FC<MealPlanningProps> = ({ 
  ...
  const [activeTab, setActiveTab] = useState<'plano' | 'criar' | 'estrategia' | 'substituicoes' | 'distribuicao' | 'orientacoes' | 'ia'>('plano');
  ...
  const totals = useMemo(() => {
    if (!activePlan || !activePlan.meals) return { calories: 0, protein: 0, carbs: 0, fats: 0 };
    return activePlan.meals.reduce((acc, meal) => {
  ...
  const handleRemoveFood = (mealIdx: number, foodIdx: number) => {
    if (!activePlan || !activePlan.meals) return;
    const newMeals = [...activePlan.meals];
    newMeals[mealIdx].foods = newMeals[mealIdx].foods.filter((_, i) => i !== foodIdx);
    setActivePlan({ ...activePlan, meals: newMeals });
  };
  ...
  const handleAddMeal = () => {
    ...
    const newMeal: Meal = {
      ...
      time: '00:00',
      foods: [],
      order_index: (activePlan.meals?.length || 0) + 1
    };
    setActivePlan({ 
      ...activePlan, 
    ...
    <div className="space-y-8">
      <div className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm">
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            <h3 className="text-2xl font-black text-[#0B2B24]">Sistema de Substituições</h3>
            <p className="text-slate-400 font-medium">Gerencie equivalências para dar flexibilidade ao paciente.</p>
          </div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="p-8 bg-slate-50 rounded-[2.5rem] border border-slate-100 space-y-6">
            <h4 className="text-lg font-black text-[#0B2B24]">Grupos de Substituição</h4>
            <div className="space-y-3">
              {['Proteínas Magras', 'Carboidratos Complexos', 'Vegetais A', 'Frutas'].map((group) => (
                <div key={group} className="flex items-center justify-between p-4 bg-white rounded-2xl border border-slate-100 group hover:border-[#22B391] transition-all cursor-pointer">
          ...
          <div className="p-8 bg-white rounded-[2.5rem] border border-slate-100 shadow-sm space-y-6">
            <div className="flex items-center justify-between">
              <h4 className="text-lg font-black text-[#0B2B24]">Equivalentes</h4>
              <span className="px-3 py-1 bg-[#22B391]/10 text-[#22B391] rounded-full text-[10px] font-black uppercase tracking-widest">Proteínas Magras</span>
            </div>
            <div className="space-y-4">
              {[
                ...
                <div key={item.name} className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-50">
                  <span className="font-bold text-slate-600">{item.name}</span>
                  <span className="text-sm font-black text-[#0B2B24]">{item.qty}</span>
                </div>
              ))}
            </div>
          </div>
    ...
    <div className="space-y-8">
      <div className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm">
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            <h3 className="text-2xl font-black text-[#0B2B24]">Distribuição de Macronutrientes</h3>
            <p className="text-slate-400 font-medium">Análise visual da composição nutricional do plano.</p>
          </div>
        </div>
          ...
          <div className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
                  data={[
                    { name: 'Proteína', value: totals.protein * 4, color: '#3b82f6' },
                    { name: 'Carboidrato', value: totals.carbs * 4, color: '#10b981' },
                    { name: 'Gordura', value: totals.fats * 9, color: '#f59e0b' },
                  ]}
                  cx="50%"
                  cy="50%"
                  innerRadius={60}
          ...
          <div className="flex flex-col justify-center space-y-6">
            <div className="space-y-2">
              <div className="flex justify-between text-sm font-black uppercase tracking-widest">
                ...
                <span className="text-[#0B2B24]">{Math.round((totals.protein * 4 / totals.calories) * 100)}%</span>
              </div>
              <div className="w-full h-3 bg-slate-100 rounded-full overflow-hidden">
                <div className="h-full bg-blue-500" style={{ width: `${(totals.protein * 4 / totals.calories) * 100}%` }} />
              </div>
            </div>
            <div className="space-y-2">
              <div className="flex justify-between text-sm font-black uppercase tracking-widest">
                <span className="text-emerald-500">Carboidratos</span>
                <span className="text-[#0B2B24]">{Math.round((totals.carbs * 4 / totals.calories) * 100)}%</span>
              </div>
              <div className="w-full h-3 bg-slate-100 rounded-full overflow-hidden">
                <div className="h-full bg-emerald-500" style={{ width: `${(totals.carbs * 4 / totals.calories) * 100}%` }} />
              </div>
            </div>
            <div className="space-y-2">
              <div className="flex justify-between text-sm font-black uppercase tracking-widest">
                <span className="text-orange-500">Gorduras</span>
                <span className="text-[#0B2B24]">{Math.round((totals.fats * 9 / totals.calories) * 100)}%</span>
              </div>
              <div className="w-full h-3 bg-slate-100 rounded-full overflow-hidden">
                <div className="h-full bg-orange-500" style={{ width: `${(totals.fats * 9 / totals.calories) * 100}%` }} />
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-8">
          <div className="text-center">
            <p className="text-sm font-black text-[#0B2B24]">{activePlan?.target_calories || 0} kcal</p>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Meta</p>
          </div>
          <div className="h-8 w-px bg-slate-100" />
          <div className="text-center">
            <p className="text-sm font-black text-[#22B391]">{Math.round(totals.calories)} kcal</p>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Prescrito</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {isEditing ? <Save className="w-5 h-5" onClick={handleSavePlan} /> : <Edit2 className="w-5 h-5" />}
          </button>
      ...
      <div className="grid grid-cols-1 gap-6">
            ...
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex items-center gap-4">
                <div className="text-right">
                  <p className="text-sm font-black text-[#0B2B24]">{Math.round(meal.foods.reduce((s, f) => s + Number(f.calories), 0))} kcal</p>
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-slate-50">
                  {meal.foods.map((food, fIdx) => (
                    <tr key={food.id || fIdx} className="group hover:bg-slate-50/50 transition-all">
                      <td className="py-4">
                        {isEditing ? (
                          <input 
                            type="text" 
                      ...
                      <td className="py-4">
                        {isEditing ? (
                          <input 
                            ...
                            onChange={(e) => {
                              const newMeals = [...(activePlan.meals || [])];
                              newMeals[idx].foods[fIdx].quantity = Number(e.target.value);
                              setActivePlan({...activePlan, meals: newMeals});
                            }}
                            className="w-16 bg-transparent font-black text-[#0B2B24] outline-none"
                          />
                        ) : (
                          <span className="font-black text-[#0B2B24]">{food.quantity}</span>
                        )}
                      </td>
                      <td className="py-4">
                        {isEditing ? (
                          ...
                          <span className="text-sm font-bold text-slate-400">{food.unit}</span>
                        )}
                      </td>
                      <td className="py-4">
                        {isEditing ? (
                          <input 
                            ...
                            onChange={(e) => {
                              const newMeals = [...(activePlan.meals || [])];
                              newMeals[idx].foods[fIdx].protein = Number(e.target.value);
                              setActivePlan({...activePlan, meals: newMeals});
                            }}
                            className="w-12 bg-transparent text-sm font-bold text-blue-500 outline-none"
                          />
                        ) : (
                          <span className="text-sm font-bold text-blue-500">{food.protein}g</span>
                        )}
                      </td>
                      <td className="py-4">
                        {isEditing ? (
                          <input 
                            ...
                            onChange={(e) => {
                              const newMeals = [...(activePlan.meals || [])];
                              newMeals[idx].foods[fIdx].carbs = Number(e.target.value);
                              setActivePlan({...activePlan, meals: newMeals});
                            }}
                            className="w-12 bg-transparent text-sm font-bold text-emerald-500 outline-none"
                          />
                        ) : (
                          <span className="text-sm font-bold text-emerald-500">{food.carbs}g</span>
                        )}
                      </td>
                      <td className="py-4">
                        {isEditing ? (
                          <input 
                            ...
                            onChange={(e) => {
                              const newMeals = [...(activePlan.meals || [])];
                              newMeals[idx].foods[fIdx].fats = Number(e.target.value);
                              setActivePlan({...activePlan, meals: newMeals});
                            }}
                            className="w-12 bg-transparent text-sm font-bold text-orange-500 outline-none"
                          />
                        ) : (
                          <span className="text-sm font-bold text-orange-500">{food.fats}g</span>
                        )}
                      </td>
                      <td className="py-4 text-right">
                        {isEditing ? (
                          <input 
                            ...
                            onChange={(e) => {
                              const newMeals = [...(activePlan.meals || [])];
                              newMeals[idx].foods[fIdx].calories = Number(e.target.value);
                              setActivePlan({...activePlan, meals: newMeals});
                            }}
                            className="w-16 bg-transparent text-sm font-black text-[#0B2B24] text-right outline-none"
                          />
                        ) : (
                          <span className="text-sm font-black text-[#0B2B24]">{food.calories}</span>
                        )}
                      </td>
                      {isEditing && (
                        <td className="py-4 text-right">
        ...
        {isEditing && (
          ...
          >
            ...
            <span className="font-black uppercase tracking-widest text-xs">Adicionar Refeição</span>
          </button>
        )}
      </div>
    </div>
        ...
        {loading ? (
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              {activeTab !== 'plano' && activeTab !== 'ia' && activeTab !== 'substituicoes' && activeTab !== 'distribuicao' && (
                <div className="bg-white p-12 rounded-[3rem] border border-slate-100 shadow-sm text-center">
                  ...
                  <h3 className="text-xl font-black text-[#0B2B24] mb-2">Módulo em Desenvolvimento</h3>
                  <p className="text-slate-400 font-medium max-w-md mx-auto">
                    Estamos trabalhando para trazer a melhor experiência em {sidebarItems.find(i => i.id === activeTab)?.label}.
                  </p>
                </div>
              )}
            </motion.div>
