  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const model = ai.models.generateContent({
        ...
        contents: `Gere um plano alimentar para o paciente ${patientName}.
        Objetivo: ${preferences.dietType}
        Calorias Alvo: ${targetCalories} kcal
        Macros Alvo: Proteína ${targetMacros.protein}g, Carboidrato ${targetMacros.carbs}g, Gordura ${targetMacros.fats}g
        Número de Refeições: ${preferences.mealsCount}
        Restrições: ${preferences.restrictions}
        Preferências: ${preferences.preferences}`,
        config: {
          systemInstruction: `Você é um nutricionista especialista em prescrição dietética. 
        Gere um plano alimentar completo em formato JSON.
        O plano deve conter uma lista de refeições (meals), cada uma com nome (name), horário (time) e uma lista de alimentos (foods).
        Cada alimento deve ter: nome (name), quantidade (quantity), unidade (unit), calorias (calories), proteínas (protein), carboidratos (carbs) e gorduras (fats).
        Respeite rigorosamente as calorias alvo e a distribuição de macros.
          ...
          "meals": [
            {
              ...
              "time": "HH:MM",
              "foods": [
                { "name": "string", "quantity": number, "unit": "string", "calories": number, "protein": number, "carbs": number, "fats": number }
              ]
            }
          ]
        }`,
