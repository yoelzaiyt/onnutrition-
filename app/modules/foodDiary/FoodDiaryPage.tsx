...
import { getMealsByDate, addMeal, updateMeal, deleteMeal, getDaySummary, updateDaySummary } from "./foodDiary.service";
...
export default function FoodDiaryPage({ patientId: initialPatientId, onBack }: FoodDiaryPageProps) {
  ...
  const handleToggleStatus = async (mealId: string, currentStatus: boolean) => {
    if (!patientId) return;
    try {
      const { error } = await updateMeal(patientId, mealId, { consumed: !currentStatus });
      if (error) throw error;
      setMeals(meals.map(m => m.id === mealId ? { ...m, consumed: !currentStatus } : m));
    } catch (error) {
  ...
  const handleDeleteMeal = async (mealId: string) => {
    if (!patientId || !confirm('Tem certeza que deseja excluir este registro?')) return;
    try {
  ...
  const progressPercent = summary ? Math.min(Math.round((totals.calories / summary.targetCalories) * 100), 100) : 0;
        ...
        <div className="bg-[#0B2B24] text-white p-8 rounded-[40px] shadow-2xl shadow-emerald-900/20 relative overflow-hidden">
          ...
          <div className="relative z-10">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-8">
              <div>
                <h1 className="text-3xl font-black tracking-tight mb-1">Resumo do Dia</h1>
                <p className="text-emerald-400/60 text-[10px] font-black uppercase tracking-[0.2em]">
                  {new Date(selectedDate).toLocaleDateString('pt-BR', { weekday: 'long', day: '2-digit', month: 'long', year: 'numeric' })}
                </p>
              </div>
              ...
              <div className="md:col-span-1 flex flex-col items-center justify-center text-center">
                <div className="relative w-32 h-32 flex items-center justify-center">
                  <svg className="w-full h-full transform -rotate-90">
                    ...
                    <circle cx="64" cy="64" r="58" stroke="currentColor" strokeWidth="8" fill="transparent" strokeDasharray={364.4} strokeDashoffset={364.4 - (364.4 * progressPercent) / 100} className="text-[#22B391] transition-all duration-1000" />
                  </svg>
                  <div className="absolute inset-0 flex flex-col items-center justify-center">
                    <span className="text-2xl font-black leading-none">{totals.calories}</span>
                    <span className="text-[8px] font-black uppercase tracking-widest opacity-40">kcal</span>
                  </div>
                </div>
                <p className="mt-4 text-[10px] font-black uppercase tracking-widest opacity-60">Meta: {summary?.targetCalories} kcal</p>
              </div>

              <div className="md:col-span-3 grid grid-cols-3 gap-4">
                {[
                  { label: 'Proteínas', value: totals.protein, target: summary?.targetProtein, color: 'bg-emerald-500' },
                  { label: 'Carbos', value: totals.carbs, target: summary?.targetCarbs, color: 'bg-blue-500' },
                  { label: 'Gorduras', value: totals.fat, target: summary?.targetFat, color: 'bg-amber-500' },
                ].map((macro) => (
                  <div key={macro.label} className="bg-white/5 p-5 rounded-3xl border border-white/5">
                    <p className="text-[9px] font-black uppercase tracking-widest opacity-40 mb-2">{macro.label}</p>
                    <div className="flex items-end gap-1 mb-3">
                      <span className="text-xl font-black">{macro.value}</span>
                      <span className="text-[8px] font-black opacity-40 mb-1">g</span>
                    </div>
                    ...
                    <p className="mt-2 text-[8px] font-black opacity-40">Meta: {macro.target}g</p>
                  </div>
                ))}
              </div>
            </div>
              ...
              <div key={mealType.type} className="space-y-4">
                <div className="flex items-center justify-between px-4">
                  <div className="flex items-center gap-3">
                    ...
                    <h2 className="text-xl font-black text-[#0B2B24] tracking-tight">{mealType.label}</h2>
                  </div>
                  <button 
                    onClick={() => {
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {mealEntries.length > 0 ? (
                    ...
                    <div className="col-span-full border-2 border-dashed border-slate-100 rounded-[2rem] p-8 flex flex-col items-center justify-center text-center group hover:border-emerald-100 transition-all">
                      <p className="text-xs font-bold text-slate-300 group-hover:text-emerald-400 transition-colors">Nenhum registro para esta refeição</p>
                    </div>
                  )}
                </div>
              </div>
        ...
        <div className="bg-white p-8 rounded-[40px] border border-slate-100 shadow-sm">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-xl font-black text-[#0B2B24] tracking-tight">Observações do Nutricionista</h2>
            </div>
            <button 
      ...
      <div className="w-full lg:w-80 space-y-8">
        ...
        <div className="bg-white p-8 rounded-[40px] border border-slate-100 shadow-sm">
          <div className="flex items-center gap-3 mb-8">
            ...
            <h2 className="text-lg font-black text-[#0B2B24] tracking-tight">Análise de Macros</h2>
          </div>
            ...
            {macroData.map((macro) => (
              <div key={macro.name} className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-bold text-slate-500">{macro.name}</span>
                </div>
                <span className="text-xs font-black text-[#0B2B24]">{macro.value}g</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-rose-50 p-8 rounded-[40px] border border-rose-100">
          <div className="flex items-center gap-3 mb-6">
            ...
            <h2 className="text-lg font-black text-rose-900 tracking-tight">Alertas</h2>
          </div>
          ...
          <div className="space-y-4">
            ...
            {totals.protein < (summary?.targetProtein || 0) * 0.5 && (
              <div className="flex gap-3">
                <div className="w-1 h-auto bg-rose-300 rounded-full shrink-0" />
                <p className="text-xs font-bold text-rose-800 leading-relaxed">
                  Consumo de proteínas muito baixo. Tente incluir mais fontes proteicas.
                </p>
              </div>
            )}
            {totals.calories === 0 && (
              <p className="text-xs font-bold text-rose-400 italic">Nenhum alerta para os dados atuais.</p>
            )}
          </div>
        </div>
        ...
        <div className="bg-white p-8 rounded-[40px] border border-slate-100 shadow-sm">
          <div className="flex items-center gap-3 mb-8">
            ...
            <h2 className="text-lg font-black text-[#0B2B24] tracking-tight">Histórico</h2>
          </div>
              ...
              return (
                <button 
                  ...
                  className={`w-full flex items-center justify-between p-4 rounded-2xl transition-all ${
                    selectedDate === dStr ? 'bg-emerald-50 text-[#22B391] border border-emerald-100' : 'hover:bg-slate-50 text-slate-400'
                  }`}
                >
                  <span className="text-xs font-black uppercase tracking-widest">
                    {i === 0 ? 'Hoje' : i === 1 ? 'Ontem' : d.toLocaleDateString('pt-BR', { day: '2-digit', month: 'short' })}
                  </span>
        ...
        <div className="bg-gradient-to-br from-[#0B2B24] to-[#22B391] p-8 rounded-[40px] text-white relative overflow-hidden group">
          ...
          <div className="relative z-10">
            ...
            <h3 className="text-lg font-black tracking-tight mb-2">ONNutrition AI</h3>
            <p className="text-xs font-bold text-white/60 leading-relaxed">
              Em breve: Recomendações automáticas baseadas no seu perfil e objetivos.
            </p>
          </div>
        </div>

        {/* Seed Data Button (Debug) */}
        <div className="bg-slate-50 p-6 rounded-[40px] border border-slate-100 border-dashed">
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4 text-center">Modo Desenvolvedor</p>
          <button 
            onClick={handleSeedData}
            disabled={isLoading}
            className="w-full py-3 bg-white border border-slate-200 rounded-2xl text-xs font-black text-slate-500 hover:bg-[#22B391] hover:text-white hover:border-[#22B391] transition-all flex items-center justify-center gap-2"
          >
